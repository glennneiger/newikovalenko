...
(function( $ ) {
    ...
    Marker.prototype.setSrc = function(src, mapScale){
        ...
        img.onload = function(){
            ...
            marker.x = marker.x - (this.width - marker.width)/2; // + marker.x + marker.width/2
            marker.y = marker.y - (this.height - marker.height); // marker.y + marker.height
    ...
    Marker.prototype.moveToClick = function(xy){
        ...
        var _data = this.mapsvg.getData();
        var options = this.mapsvg.getOptions();
        var markerOptions = {};

        xy[0] = xy[0]/_data.scale - this.width/2 + _data.viewBox[0];
        xy[1] = xy[1]/_data.scale - this.height  + _data.viewBox[1];


        if(_data.mapIsGeo)
            this.geoCoords = this.mapsvg.convertPixelToGeo(this.x + this.width/2, this.y + (this.height-1));
    ...
    Marker.prototype.setGeoCoords = function(coords){
        ...
        if(typeof coords == 'object' && coords.length==2){
            if($.isNumeric(coords[0]) && $.isNumeric(coords[1])){
                var xy = this.mapsvg.convertGeoToPixel(coords);
                xy[0] = xy[0] - this.width / 2 + .3;
                xy[1] = (xy[1] - this.height) + 1.5;
                this.setXy(xy);
            }
        }
    };
    ...
    // var pluginCSSURL   =  pluginRootURL+'css/';
    //
    // MapSVG.urls = {
    //     js: pluginJSURL,
    //     css: pluginCSSURL,
    ...
    var defaults = {
        ...
        colors              : {baseDefault: "#000000", background: "#eeeeee", selected: 40, hover: 20, directory: '#fafafa', status: {}},
        regions             : {},
        markers             : [],
        ...
        beforeLoad          : null,
        afterLoad           : null,
        zoom                : {on: false, limit: [0,10], delta: 1.15, buttons: {on: true, location: 'right'}, mousewheel: true},
        scroll              : {on: false, limit: false, background: false},
        responsive          : true,
        tooltips            : {on: false, position: 'bottom-right', template: ''},
        popovers            : {on: false, position: 'top', template: '', centerOn: false},
        multiSelect         : false,
        regionStatuses      : {
    ...
    mapSVG = function(elem, options){
        ...
        this.methods = {
            ...
            getPagination : function(callback){
                ...
                if(_this.database.onFirstPage() && !_this.database.hasMoreRecords){
                    _data.pager.hide();
                }else{
                ...
                _data.pager.on('click','.mapsvg-next:not(.disabled)',function(e){
                    e.preventDefault();
                    if(!_this.database.hasMoreRecords)
                        return;
            ...
            setEvents: function(options){
                $.extend(true, _data.options.events, options);
            },
            ...
            setTooltips : function (options) {
                ...
                if(!_data.mapTip){
                    ...
                    _data.layers.popovers.append(_data.mapTip);
                }
                ...
                switch (_data.options.tooltips.position){
                    case 'bottom-right':
                        $('body').on(event, function(e) {
                            ...
                            // var nx = pos[0] - _data.layers.popovers.offset().left;
                            // var ny = _data.$map.height() - (pos[1] - _data.$map.offset().top - 12);

                            tip.style.left = (e.clientX + $(window).scrollLeft() - _data.layers.popovers.offset().left) +'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() + 25 - _data.$map.offset().top) +'px';
                        });
                        break;
                    case 'bottom-left':
                        $('body').on(event, function(e) {
                            window.m = mouseCoords(e);
                            tip.style.left = (e.clientX + $(window).scrollLeft() - tip.offsetWidth + 10 - _data.layers.popovers.offset().left) +'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() + 25 - _data.$map.offset().top) +'px';
                        });
                        break;
                    case 'bottom':
                        $('body').on(event, function(e) {
                            window.m = mouseCoords(e);
                            tip.style.left = (e.clientX + $(window).scrollLeft() - tip.offsetWidth/2  - _data.layers.popovers.offset().left) + 'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() + 25  - _data.$map.offset().top) + 'px';
                        });
                        break;
                    case 'top':
                        ...
                        $('body').on(event, function(e) {
                            window.m = mouseCoords(e);
                            tip.style.left = (e.clientX + $(window).scrollLeft() - tip.offsetWidth/2 - _data.layers.popovers.offset().left) + 'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() - tip.offsetHeight - 10  - _data.$map.offset().top) + 'px';
                        });
                        break;
                    case 'top-right':
                        $('body').on(event, function(e) {
                    ...
                    case 'left':
                        $('body').on(event, function(e) {
                            window.m = mouseCoords(e);
                            tip.style.left = (e.clientX + $(window).scrollLeft() - tip.offsetWidth - 10 - _data.layers.popovers.offset().left) + 'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() - tip.offsetHeight/2  - _data.$map.offset().top) + 'px';
                        });
                        break;
                    case 'right':
                        $('body').on(event, function(e) {
                            window.m = mouseCoords(e);
                            tip.style.left = (e.clientX + $(window).scrollLeft() + 15 - _data.layers.popovers.offset().left) + 'px';
                            tip.style.top = (e.clientY + $(window).scrollTop() - tip.offsetHeight/2 - _data.$map.offset().top) + 'px';
                        });
                        break;
                }
            ...
            setPopovers : function (options){
                ...
                if(!_data.mapPopover) {
                    ...
                    _data.mapPopover.closeButton.on('click', function(){
                        _this.hidePopover();
                ...
                if(_data.options.popovers.mobileFullscreen && !_data.mapPopover.hasClass('mapsvg-popover-fullscreen'))
                    _data.mapPopover.addClass('mapsvg-popover-fullscreen')
                ...
                $.each(_data.zoomLevels, function(key, level){
                    if(viewBoxPrev.length){
                        if(bbox[2]>bbox[3] && ((viewBoxPrev[2] > bbox[2]) && (bbox[2] > level.viewBox[2]))){
                            _data.zoomLevel = parseInt(key)-padding;
            ...
            markerAddFinalStep : function(options, create){
                ...
                if (options.xy || (MapSVG.isNumber(options.x) && MapSVG.isNumber(options.y))){
                    ...
                    var xy = _this.convertGeoToPixel(options.geoCoords);
            ...
            regionClickHandler : function(e, region, skipPopover){
                ...
                if(region.mapsvg_type == 'region'){
                    ...
                    _this.selectRegion(region.id);
                    var regionData = region.forTemplate();

                    if(actions.region.click.zoom){
                        _this.zoomTo(region, 6);
                    }

                    if(!_data.editMode && actions.region.click.showDetails){
                        // if(actions.region.click.showDetailsFor == 'databaseObject')
                        //     _this.loadDetailsView(region.objects[0]);
                        // else{
                    ...
                    if(actions.region.click.showPopover){
                        _this.hidePopover();
                        var popover;
                        var passingObject = actions.region.click.showPopoverFor == 'region' ? regionData : regionData.objects[0];
                        popover = _data.templates.popoverRegion(passingObject);
                    ...
                    if(_data.options.events['click.region'])
                        _data.options.events['click.region'].call(region, e, _this);
                    ...
                    if(actions.marker.click.showPopover){
                        _this.hidePopover();
                        popover = _data.templates.popoverMarker(passingObject);
                        popover && _this.showPopover(e, popover, null, region);
                    }
                    ...
                    if(_data.options.events['click.marker'])
                        _data.options.events['click.marker'].call(region, e, _this);
                }
            },
            ...
            hideMarkersExceptOne: function(id){
                _data.markers.forEach(function(m){
                    if(m.id!=id)
                        m.attr('class', m.attr('class')+' mapsvg-marker-hidden');
                });
            },
            showMarkers: function(){
                _data.markers.forEach(function(m){
                    m.attr('class', m.attr('class').replace(' mapsvg-marker-hidden',''));
                });
            },
            markerAddClickHandler : function(e){
                ...
                var data = {xy: [x, y], group: _data.editMarkers.group};
            ...
            showPopover : function (e, content, pos, mapObject){
                ...
                if(mapObject && _data.options.popovers.centerOn){
                    _this.centerOn(mapObject);
                }
                ...
                if(content){
                    ...
                    if(_data.options.events['shown.popover']){
                        _data.options.events['shown.popover'].call(_data.mapPopover, mapObject, _this);
                    }
                }else{
                    _data.mapPopover.removeClass('mapsvg-popover-animate');
            ...
            mouseOverHandler : function(e){
                ...
                if(this instanceof Region) {
                    if (!this.selected)
                        this.highlight();
                    if(_data.options.events['mouseover.region'])
                        _data.options.events['mouseover.region'].call(this, e, _this);
                }else{
                    if(_data.options.events['mouseover.marker'])
                        _data.options.events['mouseover.marker'].call(this, e, _this);
            ...
            mouseOutHandler : function(e){
                ...
                if(this instanceof Region) {
                    if (!this.selected)
                        this.unhighlight();
                    if(_data.options.events['mouseout.region'])
                        _data.options.events['mouseout.region'].call(this, e, _this);
                }else{
                    if(_data.options.events['mouseout.marker'])
                        _data.options.events['mouseout.marker'].call(this, e, _this);
                }
            },
            ...
            addLayer: function(name){
                ...
                _data.$map.append(_data.layers[name]);
            },
            ...
            init: function(opts, elem) {
                ...
                if(opts.events && opts.events['beforeLoad'] && typeof opts.events['beforeLoad'] === 'function'){
                    try{
                        opts.events['beforeLoad'].call( _this)
                    }catch(err){}
                ...
                $.ajax({
                    ...
                    success:  function(xmlData){
                        ...
                        _this.database = new MapSVG.DatabaseService({
                            ...
                            sortBy    : _data.options.menu.sortBy,
                            sortDir   : _data.options.menu.sortDirection,
                            table     : 'database'
                        }, _this);
                        _data.options.events['databaseLoaded'] && _this.database.on('dataLoaded', _data.options.events['databaseLoaded']);
                        ...
                        _data.options.events['regionsLoaded'] && _this.regionsDatabase.on('dataLoaded', _data.options.events['regionsLoaded']);
                        ...
                        _this.database.getAll().done(function (data) {
                            _this.regionsDatabase.getAll().done(function(regions){
                                ...
                                if(_data.options.events['afterLoad'] && typeof _data.options.events['afterLoad'] === 'function'){
                                    try{
                                        _data.options.events['afterLoad'].call(_this);
                                    }catch (err){
                                    }
                                ...
                                // _this.setMarkersClickAsLink();
                                // TODO remove duplicate afteLoad
